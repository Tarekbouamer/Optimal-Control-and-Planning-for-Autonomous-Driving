...
ACADOvariables nmpcVariables;
/** Instance of the private workspace structure. */
ACADOworkspace nmpcWorkspace;

#if ACADO_QP_SOLVER == ACADO_FORCES
#include "forces.h"
extern forces_info nmpcForces_info;
					...
					)
{
	...
	getArray(1, src, 0, "x", nmpcVariables.x, ACADO_N + 1, ACADO_NX);
	getArray(1, src, 0, "u", nmpcVariables.u, ACADO_N, ACADO_NU);
	
#if ACADO_NXA	
	getArray(1, src, 0, "z", nmpcVariables.z, ACADO_N, ACADO_NXA);
#endif	
	
#if ACADO_NOD
	getArray(1, src, 0, "od", nmpcVariables.od, ACADO_N + 1, ACADO_NOD);
#endif
	
	getArray(1, src, 0, "y", nmpcVariables.y, ACADO_N, ACADO_NY);
	
#if ACADO_NYN
	getArray(1, src, 0, "yN", nmpcVariables.yN, 1, ACADO_NYN);
#endif /* ACADO_NYN */

#if ACADO_INITIAL_STATE_FIXED
	getArray(1, src, 0, "x0", nmpcVariables.x0, 1, ACADO_NX);
#endif /* ACADO_INITIAL_STATE_FIXED */

#if ACADO_WEIGHTING_MATRICES_TYPE == 1
	getArray(1, src, 0, "W", nmpcVariables.W, ACADO_NY, ACADO_NY);
	getArray(1, src, 0, "WN", nmpcVariables.WN, ACADO_NYN, ACADO_NYN);
#elif ACADO_WEIGHTING_MATRICES_TYPE == 2
	getArray(1, src, 0, "W", nmpcVariables.W, ACADO_N * ACADO_NY, ACADO_NY);
	getArray(1, src, 0, "WN", nmpcVariables.WN, ACADO_NYN, ACADO_NYN);
	...
	getArray(1, src, 0, "lbValues", nmpcVariables.lbValues, ACADO_N * ACADO_NU, 1);
	getArray(1, src, 0, "ubValues", nmpcVariables.ubValues, ACADO_N * ACADO_NU, 1);
#else
	getArray(1, src, 0, "lbValues", nmpcVariables.lbValues, ACADO_NX + ACADO_N * ACADO_NU, 1);
	getArray(1, src, 0, "ubValues", nmpcVariables.ubValues, ACADO_NX + ACADO_N * ACADO_NU, 1);
#endif /* ACADO_INITIAL_STATE_FIXED == 0 */

#if QPOASES_NCMAX > 0
	/* Affine constraints */
	getArray(1, src, 0, "lbAValues", nmpcVariables.lbAValues, QPOASES_NCMAX, 1);
	getArray(1, src, 0, "ubAValues", nmpcVariables.ubAValues, QPOASES_NCMAX, 1);
#endif /* QPOASES_NCMAX > 0 */

#elif (ACADO_HARDCODED_CONSTRAINT_VALUES == 0) && (ACADO_QP_SOLVER == ACADO_FORCES) && !ACADO_BLOCK_CONDENSING
	/* Bounds */
	#if (ACADO_QP_NLB > 0)
	getArray(1, src, 0, "lbValues", nmpcVariables.lbValues, ACADO_QP_NLB, 1);
	#endif
	#if (ACADO_QP_NUB > 0)
	getArray(1, src, 0, "ubValues", nmpcVariables.ubValues, ACADO_QP_NUB, 1);
	#endif

#elif (ACADO_HARDCODED_CONSTRAINT_VALUES == 0) && (ACADO_QP_SOLVER == ACADO_QPDUNES) && !ACADO_BLOCK_CONDENSING
	/* Bounds */
	getArray(1, src, 0, "lbValues", nmpcVariables.lbValues, ACADO_QP_NV, 1);
	getArray(1, src, 0, "ubValues", nmpcVariables.ubValues, ACADO_QP_NV, 1);
	...
	#if (ACADO_QP_NCA > 0)
	/* Affine bounds */
	getArray(1, src, 0, "lbAValues", nmpcVariables.lbAValues, ACADO_QP_NCA, 1);
	getArray(1, src, 0, "ubAValues", nmpcVariables.ubAValues, ACADO_QP_NCA, 1);
	#endif
#elif (ACADO_HARDCODED_CONSTRAINT_VALUES == 0) && (ACADO_QP_SOLVER == ACADO_GENERIC)
	/* Bounds */
	getArray(1, src, 0, "lbValues", nmpcVariables.lbValues, ACADO_N*(ACADO_NX+ACADO_NU), 1);
	getArray(1, src, 0, "ubValues", nmpcVariables.ubValues, ACADO_N*(ACADO_NX+ACADO_NU), 1);
	#if (ACADO_NPAC > 0)
	/* Affine bounds */
	getArray(1, src, 0, "lbAValues", nmpcVariables.lbAValues, ACADO_N*ACADO_NPAC, 1);
	getArray(1, src, 0, "ubAValues", nmpcVariables.ubAValues, ACADO_N*ACADO_NPAC, 1);
	#endif
#endif

#if ACADO_USE_ARRIVAL_COST == 1
	getArray(1, src, 0, "xAC", nmpcVariables.xAC, ACADO_NX, 1);
	getArray(1, src, 0, "SAC", nmpcVariables.SAC, ACADO_NX, ACADO_NX);
    getArray(1, src, 0, "WL",  nmpcVariables.WL,  ACADO_NX, ACADO_NX);
#endif
	...
	switch ( ctrl )
	{
		case 0:
			...
			if ( !initialized )
			{
				...
				if (initType == 1)
				{
					nmpc_initializeNodesByForwardSimulation();
				}
				
#if ACADO_USE_ARRIVAL_COST == 1 
                nmpc_updateArrivalCost( 1 );
			...
			{
#if ACADO_USE_ARRIVAL_COST == 1 
                nmpc_updateArrivalCost( 0 );
#endif /* ACADO_USE_ARRIVAL_COST == 1 */
				
				nmpc_shiftStates(strategy, xEnd, uEnd);
				nmpc_shiftControls(uEnd);
			}
			
			nmpc_preparationStep();
			
			status = (real_t)nmpc_feedbackStep();
			
			kktValue = nmpc_getKKT();
			objValue = nmpc_getObjective();

#if ( (ACADO_QP_SOLVER == ACADO_QPOASES) || (ACADO_QP_SOLVER == ACADO_QPOASES3) )
			nIterations = (double)nmpc_getNWSR();
#elif ACADO_QP_SOLVER == ACADO_FORCES
			nIterations = nmpcForces_info.it;
#elif ACADO_QP_SOLVER == ACADO_QPDUNES
	...
	setArray(plhs[ 0 ], 0, "x", nmpcVariables.x, ACADO_N + 1, ACADO_NX);
	setArray(plhs[ 0 ], 0, "u", nmpcVariables.u, ACADO_N, ACADO_NU);
#if ACADO_NXA > 0
	setArray(plhs[ 0 ], 0, "z", nmpcVariables.z, ACADO_N, ACADO_NXA);
#endif	
		
#if ACADO_USE_ARRIVAL_COST == 1
	setArray(plhs[ 0 ], 0, "xAC", nmpcVariables.xAC, ACADO_NX, 1);
	setArray(plhs[ 0 ], 0, "SAC", nmpcVariables.SAC, ACADO_NX, ACADO_NX);
#endif /* ACADO_USE_ARRIVAL_COST */

#if ACADO_COMPUTE_COVARIANCE_MATRIX == 1
	setArray(plhs[ 0 ], 0, "sigmaN", nmpcVariables.sigmaN, ACADO_NX, ACADO_NX);
