...
aSfunName  = 'nmpc_solver_sfun';
aRealT     = 'double';
aHeaderFile = 'nmpc_solver_sfunction.h';
...
ACADODef.SourceFiles = { ...
	'nmpc_solver.c', ...
	'nmpc_integrator.c', ...
	'nmpc_auxiliary_functions.c', ...
	'nmpc_solver_sfunction.c' ...
	};
	
if strcmp(ACADO_QP_SOLVER, 'QPOASES')
	ACADODef.SourceFiles{end + 1} = 'nmpc_qpoases_interface.cpp';
...
elseif strcmp(ACADO_QP_SOLVER, 'QPOASES3')
	ACADODef.SourceFiles{end + 1} = 'nmpc_qpoases3_interface.c';
...
elseif strcmp(ACADO_QP_SOLVER, 'HPMPC')
	ACADODef.SourceFiles{end + 1} = 'nmpc_hpmpc_interface.c';
else
	error('Unknown QP solver')
end;

% Define header files
ACADODef.HeaderFiles = {'nmpc_common.h', aHeaderFile};
...
if (~strcmp(aSfunName, 'acado_solver_sfun')) && (strcmp(ACADO_QP_SOLVER, 'QPOASES'))
    iFlags = {'-DQPOASES_CUSTOM_INTERFACE="nmpc_qpoases_interface.hpp"'};    
end;
if strcmp(ACADO_QP_SOLVER, 'QPOASES3')
    iFlags = {'-D__CODE_GENERATION__', '-DQPOASES_CUSTOM_INTERFACE="nmpc_qpoases3_interface.h"'};    
end;
