/*
 *   nmpc_solver_sfun.cpp Simple C-MEX S-function for function call.
 *
 *   ABSTRACT:
 *     The purpose of this sfunction is to call a simple legacy
 *     function during simulation:
 *
 *        nmpc_step(ACADOinput u1[1], ACADOoutput y1[1])
 *
 *   Simulink version           : 8.7 (R2016a) 10-Feb-2016
 *   C++ source code generated on : 24-Jul-2018 22:30:52
...
/*
   ...
   def.SFunctionName = 'nmpc_solver_sfun';
   def.OutputFcnSpec = 'nmpc_step(ACADOinput u1[1], ACADOoutput y1[1])';
   def.StartFcnSpec = 'nmpc_initialize( void )';
   def.TerminateFcnSpec = 'nmpc_cleanup( void )';
   def.HeaderFiles = {'nmpc_common.h', 'nmpc_solver_sfunction.h'};
   def.SourceFiles = {'nmpc_solver.c', 'nmpc_integrator.c', 'nmpc_auxiliary_functions.c', 'nmpc_solver_sfunction.c', 'nmpc_qpoases_interface.cpp', 'qpoases\SRC\Bounds.cpp', 'qpoases\SRC\Constraints.cpp', 'qpoases\SRC\CyclingManager.cpp', 'qpoases\SRC\Indexlist.cpp', 'qpoases\SRC\MessageHandling.cpp', 'qpoases\SRC\QProblem.cpp', 'qpoases\SRC\QProblemB.cpp', 'qpoases\SRC\SubjectTo.cpp', 'qpoases\SRC\Utils.cpp', 'qpoases\SRC\EXTRAS\SolutionAnalysis.cpp'};
 ...
 */
#define S_FUNCTION_NAME                nmpc_solver_sfun
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
     ...
     */
    ssRegMdlInfo(S, "nmpc_initialize", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

    /*
     * Register reserved identifier for OutputFcnSpec
     */
    ssRegMdlInfo(S, "nmpc_step", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

    /*
     * Register reserved identifier for TerminateFcnSpec
     */
    ssRegMdlInfo(S, "nmpc_cleanup", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
    ...
    if (isSimulationTarget) {
       ...
       */
      ssRegMdlInfo(S, "nmpc_solver_sfun_wrapper_allocmem", MDL_INFO_ID_RESERVED,
                   0, 0, ssGetPath(S));

      /*
       * Register reserved identifier for freeing PWork for SimulationTarget
       */
      ssRegMdlInfo(S, "nmpc_solver_sfun_wrapper_freemem", MDL_INFO_ID_RESERVED,
                   0, 0, ssGetPath(S));

      /*
       * Register reserved identifier for StartFcnSpec for SimulationTarget
       */
      ssRegMdlInfo(S, "nmpc_solver_sfun_wrapper_start", MDL_INFO_ID_RESERVED, 0,
                   0, ssGetPath(S));

      /*
       * Register reserved identifier for OutputFcnSpec for SimulationTarget
       */
      ssRegMdlInfo(S, "nmpc_solver_sfun_wrapper_output", MDL_INFO_ID_RESERVED, 0,
                   0, ssGetPath(S));

      /*
       * Register reserved identifier for TerminateFcnSpec for SimulationTarget
       */
      ssRegMdlInfo(S, "nmpc_solver_sfun_wrapper_terminate", MDL_INFO_ID_RESERVED,
                   0, 0, ssGetPath(S));
    }
  }
...
static void mdlStart(SimStruct *S)
{
  if (isDWorkNeeded(S)) {
    ...
    __dtBusInfo[7] = 126*__dtSizeInfo[3];
    ...
    __dtBusInfo[9] = 40*__dtSizeInfo[3];
    ...
    __dtBusInfo[11] = 160*__dtSizeInfo[3];
    ...
    __dtBusInfo[15] = 64*__dtSizeInfo[3];
    ...
    __dtBusInfo[36] = ssGetBusElementOffset(S, __ACADOoutputId, 4) +
      ssGetBusElementOffset(S, __ACADOdataId, 4);
    __dtBusInfo[37] = 64*__dtSizeInfo[3];
...
static void CheckDataTypes(SimStruct *S)
{
  ...
  {
    uint32_T chk[] = { 2437345171, 2962380566, 542242467, 346141250 };

    int_T status;
    status = CheckDataTypeChecksum(S, "ACADOinput", &chk[0]);
    if (status==-1) {
      ssSetErrorStatus(S,
                       "Unexpected error when checking the validity of the Simulink Bus/StructType 'ACADOinput'");
    } else if (status==0) {
      ssSetErrorStatus(S,
                       "The Simulink Bus/StructType 'ACADOinput' definition has changed since the S-Function was generated");
    }
  }

  /* Verify Bus/StructType 'ACADOdata', specification */
  {
    uint32_T chk[] = { 1691186216, 1681790473, 3661275894, 2057173825 };

    int_T status;
    status = CheckDataTypeChecksum(S, "ACADOdata", &chk[0]);
    if (status==-1) {
      ssSetErrorStatus(S,
                       "Unexpected error when checking the validity of the Simulink Bus/StructType 'ACADOdata'");
    } else if (status==0) {
      ssSetErrorStatus(S,
                       "The Simulink Bus/StructType 'ACADOdata' definition has changed since the S-Function was generated");
    }
  }

  /* Verify Bus/StructType 'ACADOoutput', specification */
  {
    uint32_T chk[] = { 431452189, 3392331335, 3446927958, 3991995982 };

    int_T status;
    status = CheckDataTypeChecksum(S, "ACADOoutput", &chk[0]);
    if (status==-1) {
      ssSetErrorStatus(S,
                       "Unexpected error when checking the validity of the Simulink Bus/StructType 'ACADOoutput'");
    } else if (status==0) {
      ssSetErrorStatus(S,
                       "The Simulink Bus/StructType 'ACADOoutput' definition has changed since the S-Function was generated");
    }
  }
}
